...
## This module's north star

`backbone-project` applies that worldview to **project delivery**. It owns `Project`s, their `Task`s,
and `Timesheet`s of logged effort, and it earns its keep with one sharp boundary:

> **A Project is delivery intent plus cost/billing visibility. It is not the ledger, not the Customer,
> not the invoice — and it posts no GL.**

The generated 95% is the seven entities' CRUD. The interesting 5% is a single, deliberately narrow
piece of business logic: a **submitted, billable timesheet** is handed to `backbone-billing` through
one **outbound seam** (`ProjectWriteService::bill_timesheet` → `BillingPort`). billing builds the Sales
Invoice and posts the ledger; project only records the echoed `invoice_id` and rolls up its margin
numbers. Everything else the module does — logging time, snapshotting rates, instantiating templates,
completing projects — moves *no money*. That constraint is what keeps a delivery context self-contained:
turn the module off and no ledger changes. (See [ADR-0004](adr/adr-0004-project-boundary-billing-seam.md).)
...
- **It does not own cross-domain logic.** One module = one bounded context. It never reaches into
  another module's schema or entities. The Customer is *referenced* from `backbone-party`, the Sales
  Order from `backbone-selling`, and user/employee identity from `backbone-sapiens` — all by logical
  foreign key, none copied in.
- **It does not touch the general ledger.** `backbone-project` posts **no GL**. It holds roll-up
  totals for margin visibility, but money only moves when a billable timesheet crosses the one
  outbound seam into `backbone-billing`. project is not the invoice and not the ledger.
