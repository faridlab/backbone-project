...
- Unit + integration run through `metaphor dev test`. The correctness gate is the database-backed
  suite: `tests/project_golden_cases.rs` (PGC-1…4, the roll-up + template + validation cases),
  `tests/integrity_probes.rs` (IP-1…7, the race/reverse-gear probes), and
  `tests/project_billing_seam.rs` (PBSEAM-1, which drives the **real** `backbone-billing` write path via
  a `RealBilling` adapter). The generated `tests/integration_tests.rs` HTTP suite is **not** part of the
  gate.
- To exercise the CRUD surface directly: build `ProjectModule::builder().with_database(pool).build()?`,
  and drive `project.all_crud_routes()` via `axum::http::Request` + `tower::ServiceExt`. To exercise the
  domain verbs, construct `ProjectWriteService::new(pool)` with a `BillingPort` fake (see
  `tests/common/mod.rs`'s `FakeBilling`).
