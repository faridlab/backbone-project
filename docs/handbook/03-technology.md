...
The versions below are what `backbone-project` pins at **v0.1.3** (`Cargo.toml`); where behavior is
version-specific, the version is called out.
...
- **`rust_decimal`** is load-bearing here, not incidental: hours, rates, and every roll-up amount are
  `decimal` fields (money in IDR, 2 dp) — `f64` would introduce rounding drift into margin numbers.
- **Feature flags** (`Cargo.toml`): `default = []`; `events`, `auth`, `grpc`, `openapi`, `validation`
  are all off by default. The `grpc`/`proto` generators are *disabled* in `index.model.yaml`, so
  `tonic`/`prost` are present but unused by the shipped surface.
- **The billing edge is a `[dev-dependencies]`-only path dep.** `backbone-billing = { path = "…" }`
  exists **only** so `tests/project_billing_seam.rs` can drive the real billing write path. The shipped
  library has **zero normal Cargo dependency** on billing — the [seam](04-architecture.md#4b-the-domain-flow--log-time-then-bill-it-the-outbound-seam)
  crosses through the `BillingPort` trait, and the DTOs are the wire contract. This is deliberate: it
  keeps `backbone-project` independently buildable and deployable. (See
