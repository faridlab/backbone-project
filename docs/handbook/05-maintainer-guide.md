...
metaphor make entity Vendor --module project
#    …or copy an existing model (e.g. schema/models/activity_type.model.yaml) →
#    vendor.model.yaml and edit it, then register it under `imports:` in
#    schema/models/index.model.yaml.

# 2. Validate the schema before generating.
metaphor schema schema validate project

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate project --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor project
...
> `project` is your module name (auto-detected from the current directory when omitted).
...
### Step 6 in detail — wire the service into `ProjectModule`

Generation does **not** edit the composition root for you. The composition root is **`ProjectModule`,
defined inline in [`src/lib.rs`](../../src/lib.rs)** — not `src/module.rs` (that file is an orphaned
skeleton stub; `lib.rs` never declares it). Follow the pattern already used for the seven services:

```rust
// in lib.rs — the struct:
...
Ok(ProjectModule { /* … */ vendor_service })     // ← return it

// in ProjectModule::all_crud_routes():
Router::new()
    // … existing merges …
    .merge(create_vendor_routes(self.vendor_service.clone()))   // ← mount it
```

Then add the re-export near the top of `lib.rs`: `pub use application::service::VendorService;`.
(There is also a `// <<< CUSTOM` marker inside `build()` and the struct literal for hand-added,
regen-surviving fields — use it if your addition is not itself generated.)

## Changing an existing entity

1. Edit the field in `schema/models/<entity>.model.yaml` (the SSoT — never the generated struct).
2. `metaphor schema schema validate project`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> project` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration project --database-url …`).
4. Regenerate code: `metaphor schema schema generate project --target all --force`.
...
## The billing seam: the write-service pattern

This module's real business logic does not fit in a generated CRUD service, so it lives in three
hand-authored files under `src/application/service/`, kept alive across regen by `// <<< CUSTOM`
markers in the surrounding `mod.rs`:

| File | Holds |
|------|-------|
| `project_write_service.rs` | `ProjectWriteService` — the verbs `create_project`, `add_task`, `advance_task`, `instantiate_template`, `log_time`, `cancel_timesheet`, `bill_timesheet`, `complete_project`. Raw `sqlx` against `project.*`, transactional, transition-gated. |
| `project_ports.rs` | `BillingPort` (the one outbound port) + wire DTOs `InvoiceFromTimesheet`, `InvoiceLineFromTimesheet`, `InvoiceAck`, `ProjectRejected`. |
| `project_events.rs` | `ProjectEvent` variants + `ProjectEventSink` (default `LoggingSink`). |

Three rules make this pattern safe, and you must preserve all three when you extend it:

1. **Zero normal Cargo edge to the outside.** `ProjectWriteService` never imports `backbone-billing`.
   It depends on the `BillingPort` *trait*; the DTOs are the wire contract. `backbone-billing` appears
   only in `[dev-dependencies]`, used by `tests/project_billing_seam.rs` to drive the real billing
   write path. Do not add a normal dependency on a sibling module — add a port.
2. **Every state change is gated and idempotent, in one transaction.** The roll-up UPDATE carries its
   guard inline (`WHERE status='open'`), and the status transition (`submitted → billed`,
   `submitted → cancelled`) commits in the same tx as the roll-up it implies. A new verb that mutates
   money-shaped state follows suit or it will drift the roll-up. (See the two fixes recorded in
   [ADR-0004](adr/adr-0004-project-boundary-billing-seam.md)'s parking lot.)
3. **It is not wired into `ProjectModule` and has no HTTP route — by design.** The composing service
   constructs `ProjectWriteService::new(pool)`, supplies a `BillingPort` implementation, and calls the
   verbs. If you add an HTTP entry point, put the handler in a `*_custom.rs` and compose it in
   `routes/` (never expose the raw generated CRUD `PATCH` as a substitute — it bypasses the gates).

When you add a verb: put it on `ProjectWriteService`, gate it, cover it with a golden-case test under
`tests/`, and — if it crosses a module boundary — route it through a port, never a direct dependency.
