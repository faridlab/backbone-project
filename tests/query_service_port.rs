...
//! and returns correctly-mapped DTOs for the entities a sibling module would query. Reads are
//! id-keyed through the generic services; the module is tenant-agnostic (ADR-0029), so we
...
async fn qsp1_project_read_port() {
    ...
    sqlx::query(
        r#"INSERT INTO project.projects
             (id, project_name, project_type, customer_id, status, currency)
           VALUES ($1,'Atlas','external',$2,'open','IDR')"#,
    )
    .bind(project)
    .bind(customer)
    .execute(&pool)
