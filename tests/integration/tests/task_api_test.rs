...
//!
//! Tests the Task CRUD API endpoints.
...
// ============================================================================
// TASK TEST DATA GENERATOR
// ============================================================================

/// Test data generator for Task
pub struct TaskTestData;

impl TestDataGenerator for TaskTestData {
    ...
    async fn seed_dependencies(&self, api: &ApiTest) -> Vec<(String, String)> {
        ...
        if let Some(id) = super::crud_test_base::create_and_get_id(api, "/api/v1/projects", &super::project_api_test::ProjectTestData).await {
            deps.push(("project_id".to_string(), id));
        }
        deps
    }
}
...
/// Task API test suite
pub struct TaskApiTest {
    inner: GenericCrudTest<TaskTestData>,
}

impl TaskApiTest {
    pub fn new() -> Self {
        let mut config = CrudTestConfig::new("/api/v1/tasks", "Task");
        config.supports_soft_delete = true;
        Self {
            inner: GenericCrudTest::new(config, TaskTestData),
        }
    }
