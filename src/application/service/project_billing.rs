...
//!
//! The module is tenant-agnostic (ADR-0029): every read and write runs unscoped, and if the
//! composing service has bound an ambient org scope it is relayed onto each transaction this
//! service opens itself so the composed RLS policy sees it. With no ambient scope (standalone
//! deployment, jobs) the transactions stay plain.
//!
...
use super::project_write_service::{
    legacy_company_echo, relay_ambient_scope, BillPeriodOutcome, ProjectError, ProjectWriteService,
};
...
impl ProjectWriteService {
    ...
    pub async fn bill_timesheet_period(
        ...
        // Gate (1) — the billability gate: the covering per-(employee, year, month) approval cycle
        // must be approved. The row carries NO state and NO approval of its own.
        let approval = self
            .rows
            .approval_status(&self.pool, employee_id, year, month)
            .await?;
        ...
        if let Some((invoice_id, amount)) = self
            .rows
            .find_invoice_for_period(&self.pool, project_id, employee_id, year, month)
            .await?
        {
            let still_open = self
                .rows
                .list_billable_period_lines(&self.pool, project_id, employee_id, year, month)
                .await?;
        ...
        // Relay the caller's ambient org scope (ADR-0029) so the composed fence sees the stamp +
        // the project roll-up; plain under an undecorated deployment.
        relay_ambient_scope(&mut tx).await?;
        let stamp = self
            .rows
            .stamp_invoice_on_period(&mut tx, project_id, employee_id, year, month, ack.invoice_id)
            .await?;
    ...
    pub async fn unbill_invoice(
        ...
        let mut tx = self.pool.begin().await?;
        relay_ambient_scope(&mut tx).await?;
        let cleared = self.rows.clear_invoice(&mut tx, invoice_id).await?;
