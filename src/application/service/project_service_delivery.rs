...
//! both keys are globally unique origin ids (ADR-0029 strip), and both inserts are
//! `ON CONFLICT … DO NOTHING` with a re-select arm, so the second confirm of the same order
//! resolves to the first mint's ids and reports `minted: false` per line.
//!
...
//! Tenant-agnostic (ADR-0029): no scoping argument — if the composing service has bound an
//! ambient org scope it is relayed onto the tx so the composed fence sees it.
//!
...
use super::project_write_service::{
    relay_ambient_scope, ProjectError, ProjectWriteService, ServiceDeliveryLineOutcome,
...
impl ProjectWriteService {
    ...
    pub async fn mint_service_delivery(
        ...
        // Relay the caller's ambient org scope (ADR-0029) — every mint write below rides it;
        // plain under an undecorated deployment.
        relay_ambient_scope(&mut tx).await?;

        // The per-order project: at most one live project per source sales order. The insert IS
        // the idempotency probe — Some(id) means THIS call created it (and owes the template
        // materialization); None means a prior mint already had it.
        ...
        if wants_order_project {
            ...
            match inserted {
                ...
                None => {
                    ...
                    let id = self
                        .projects
                        .find_id_by_source_so(&self.pool, req.order_id)
                        .await?
        ...
        for line in &req.lines {
            let outcome = match line.rung {
                ...
                ServiceTrackingRung::TaskGlobalProject => {
                    ...
                    // The fixed project must exist and be live — identified by id alone
                    // (tenant-agnostic; the composing decorator owns isolation).
                    self.projects.find_status_by_id(&self.pool, fixed).await?
                        .ok_or_else(|| ProjectError::Invalid(
                            "the line's fixed project was not found".into(),
                        ))?;
                    let task = self.mint_line_task(
                        &mut tx, fixed, line, &format!("SO {}", req.order_number),
                    ).await?;
                ...
                ServiceTrackingRung::TaskInProject => {
                    ...
                    let task = self.mint_line_task(
                        &mut tx, project_id, line, &format!("SO {}", req.order_number),
                    ).await?;
                    ServiceDeliveryLineOutcome {
                        sale_line_id: line.sale_line_id,
    ...
    async fn mint_line_task(
        ...
        let existing = self
            .tasks
            .find_id_by_origin_sale_line(tx, line.sale_line_id)
            .await?;
        Ok((None, existing))
    }
}
